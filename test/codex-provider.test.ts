...
describe("CodexCliProvider", () => {
  ...
  test("builds launch command with CODEX_HOME env", () => {
    ...
    expect(cmd[0]).toContain("CODEX_HOME=");
  ...
  test("can be registered in ProviderRegistry", () => {
    ...
    expect(registry.get("codex-cli")).toBe(provider);
    expect(registry.listIds()).toContain("codex-cli");
  });
});
